...
def bearer_auth_factory(token):
    ...
    async def bearer_auth(request, handler):
        if not request.headers.get("Authorization") == expected_value:
...
class MusicPlayerObserver:
    ...
    def send_update(self):
        ...
        run_coroutine_threadsafe(self.__ws.send_bytes(player_state), self.__loop)
...
class RemoteControlModule(commands.Cog, CogFactory):
    ...
    def is_available(cls):
        ...
        if MUSIC_REMOTE_PORT.get() is None:
            log.info("Cannot load RemoteControlModule: MUSIC_REMOTE_PORT is not set")
            return False
    ...
    async def _handle_ws(self, request):
        ...
        access_code = request.match_info["access_code"]
