...
async def test_quit_channel_if_empty_sends_event(
    mock_ctx, mock_bot, mock_voice_client, music_module
):
    before = StubVoice(StubChannel())
    after = StubVoice(None)
    music_module._MusicModule__players[before.channel.id] = mock_voice_client
    ...
    assert event[1] == mock_voice_client
...
async def test_ensure_voice_and_non_empty_queue_throws(
    mock_ctx, mock_voice_client, music_module
):
    with pytest.raises(commands.CommandError):
