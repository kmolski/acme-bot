...
async def test_run_command_pauses_on_pause_command(rmq_control_module, voice_client):
...
async def test_run_command_resumes_on_resume_command(rmq_control_module, voice_client):
...
async def test_run_command_clears_queue_on_clear_command(
    rmq_control_module, voice_client
):
...
async def test_run_command_sets_loop_on_loop_command(rmq_control_module, voice_client):
    message = FakeAmqpMessage(
        b"""
        {
            "op": "loop",
...
async def test_remove_command_deletes_existing_entry_at_offset(
    rmq_control_module, voice_client, youtube_playlist
):
...
async def test_observer_update_sends_player_state(
    rmq_player_observer, voice_client, amqp_exchange
):
    await voice_client.set_volume(58)
